...
class existenciaController extends Controller
	...
	public function cliente_exist(Request $request)
		...
		if (count($resultado) == 0)
			{
			$client = new Client;
			$res = $client->request('GET', 'http://192.168.100.20/appserviciosnext/public/getDatos', ['json' => ['tipodocumento' => 'RUC', 'nrodocumento' => $request->input('ruc_empresa') ]]);
	...
	public function usernext_exist(Request $request)
		...
		if (count($resultado) == 0)
			{
			$client = new Client;
			$res = $client->request('GET', 'http://192.168.100.20/appserviciosnext/public/getDatos', ['json' => ['tipodocumento' => 'RUC', 'nrodocumento' => $request->input('ruc') ]]);
			$respuesta = json_decode($res->getBody() , true);
			return response()->json(["respuesta" => $respuesta], 200);
			}
		  else
